// From http://matt.west.co.tt/spectrum/jsspeccy/
// with ZX81 pixel generation by Erik Olofsen

var ipxl=0, liney=-1;
var phsygen=false, pnmigen=false, lfill=false;
var radj;
var ipxl2 = 192*32*8*4;
var ppc = 0;
var rfill=255, gfill=255, bfill=255;
var chrmsk = 0x7fff;
var calls = {};
...
function z80_do_opcodes()
{
  ...
  while (event_next_event==0 || tstates < event_next_event) {

    var opcode, opcode2;
    var eol;
    var v, mask, px, pxi, pxp;
    var empty;
    var color, bright, ink, paper, irpx, igpx, ibpx, prpx, pgpx, pbpx;

    if (z80.pc == brkaddr) return;

    if (hsygen != phsygen) {
      phsygen = hsygen;
      ppc = 0;
      if (hsygen) { liney=0; grstart=true; } else { break; }
    }
    ...
    if (!nmigen && z80.halted) z80_interrupt();

    if (nmigen && tstates>=211*speed) { z80_nmi(); break; }

    tstates += 4;
    if (mtmode && expanded)
      opcode = readbyte_internal( z80.pc );
    else
      opcode = readbyte_internal( z80.pc & chrmsk );

    if (z80.pc&0x8000) {

//      if ((hrgrom==2) && (z80.r==0x7e) && (z80.i&1)) opcode = 0x76;

      eol = (opcode==0x76) && ((ipxl&1023)==0);

      if ( (mtmode==0 && opcode&0x40) || eol ) {

	if (opcode!=0x7f) {

	liney++;

	if (ipxl&1023) lfill = true;
	empty = (z80.pc == ppc+1) && !hires;

	if (lfill || empty) {
	  while (((ipxl&1023)||empty) && ipxl<ipxl2) {
	    imageDataData[ipxl++] = rfill;
	    imageDataData[ipxl++] = gfill;
	    imageDataData[ipxl++] = bfill;
	    ipxl++;
	    empty = false;
	  }
	}
	...
	lfill = false;
	if (liney%8==0) ppc = z80.pc;
        if (ipxl >= ipxl2) ipxl=0;
	...
	rstart = (z80.r&0x7f) | (z80.r7&0x80);
	if (!(ipxl&1023) && !arx) hires = (rstart!=223 && z80.i>=0x20);

	if (grstart) {
	  grstart = false;
	  if (hires) liney = 2; else if (arx) liney=0;
	  opacity = bopacity;
	}

	if (lfill) {
	  while (ipxl&1023 && ipxl<ipxl2) {
	    imageDataData[ipxl++] = rfill;
	    imageDataData[ipxl++] = gfill;
	    imageDataData[ipxl++] = bfill;
	    ipxl++;
	...
	if (z80.i==0 && cscheme==2) {
	  v = font[((opcode&63)<<3)|(liney%8)];
	} else if (mtmode) {
	  v = opcode;
//	} else if ((hrgrom==2) && (z80.i&1)) {
//	  v = memory[z80.pc];
	} else if (hires) {
	  radj = (z80.r&0x7f) | (z80.r7&0x80);
	  v = readbyte_internal((z80.i<<8)|radj);
	} else if (cscheme==3) {
	  opcode2 = ((opcode&0x80)>>1) | (opcode&0x3f);
	  v = fontqs[(opcode2<<3)|(liney%8)];
	} else if (z80.i==0x31 && !arx) {
	  opcode2 = ((opcode&0x80)>>1) | (opcode&0x3f);
	  v = readbyte_internal(((z80.i&0xfe)<<8)|(opcode2<<3)|(liney%8));
	  opcode ^= 0x80;
	} else {
	  v = readbyte_internal(((z80.i&0xfe)<<8)|((opcode&63)<<3)|(liney%8));
	}

	mask = 0x80;

	if (cscheme==0 || cscheme==3) {

        if ((!mtmode && (opcode & 0x80)) || mtmode==3) {
	  pxi = 255; pxp = 0;
	} else {
	  pxi = 0; pxp = 255;
	}
    ...
    if (green_monitor) {
	  ...
	  if (chromamode) {
            opcode2 = ((opcode&0x80)>>1) | (opcode&0x3f);
            if (chromamode&0x10)
	      color = readbyte_internal(z80.pc);
            else
	      color = readbyte_internal(0xc000|(opcode2<<3)|(liney%8));
	  } else {
	    v = font[((opcode&63)<<3)|(liney%8)];
	    color = readbyte_internal(0x2000|((z80.pc+1)&0x0fff));
	  }
          ink = color&0x0f;
          paper = (color&0xf0) >> 4;

	  switch (ink&0x07) {
	    case 0 : irpx = igpx = ibpx = 0; break;
	    case 1 : irpx = igpx = 0; ibpx = 255; break;
	    case 2 : irpx = ibpx = 0; igpx = 255; break;
	    case 3 : irpx = 0; igpx = ibpx = 255; break;
	    case 4 : irpx = 255; igpx = ibpx = 0; break;
	    case 5 : irpx = ibpx = 255; igpx = 0; break;
	    case 6 : irpx = igpx = 255; ibpx = 0; break;
	    case 7 : irpx = igpx = ibpx = 255; break;
	  }

	  switch (paper&0x07) {
	    case 0 : prpx = pgpx = pbpx = 0; break;
	    case 1 : prpx = pgpx = 0; pbpx = 255; break;
	    case 2 : prpx = pbpx = 0; pgpx = 255; break;
	    case 3 : prpx = 0; pgpx = pbpx = 255; break;
	    case 4 : prpx = 255; pgpx = pbpx = 0; break;
	    case 5 : prpx = pbpx = 255; pgpx = 0; break;
	    case 6 : prpx = pgpx = 255; pbpx = 0; break;
	    case 7 : prpx = pgpx = pbpx = 255; break;
	  }

	  if (chromamode) {
	    if (!(ink&0x08)) { irpx/=2; igpx/=2; ibpx/=2; }
	    if (!(paper&0x08)) { prpx/=2; pgpx/=2; pbpx/=2; }
	  } else {
	    if (ink&0x08) { irpx/=2; igpx/=2; ibpx/=2; }
	    if (paper&0x08) { prpx/=2; pgpx/=2; pbpx/=2; }
	  }
	  ...
	  if (chromamode) {
	    px=prpx; prpx=pgpx; pgpx=px;
	    px=irpx; irpx=igpx; igpx=px;
	  }
	  ...
	  if (opcode & 0x80) {
	    ...
	    for (var i=1; i<=8; i++) {
	      if (v & mask) {
	        imageDataData[ipxl++] = prpx;
	        imageDataData[ipxl++] = pgpx;
	        imageDataData[ipxl++] = pbpx;
	      } else {
	        imageDataData[ipxl++] = irpx;
	        imageDataData[ipxl++] = igpx;
	        imageDataData[ipxl++] = ibpx;
	      }
	      ipxl++;
	      mask >>= 1;
	    }
	...
	for (var i=1; i<=8; i++) {
	  px = (v & mask);
	  if (px && !(opcode & 0x80)) {
	    imageDataData[ipxl++] = 255;
	    imageDataData[ipxl++] = 191;
	    imageDataData[ipxl++] = 0;
	  } else if (!px && (opcode & 0x80)) {
	    imageDataData[ipxl++] = 0;
	    imageDataData[ipxl++] = 0;
	    imageDataData[ipxl++] = 255;
	  } else {
	    imageDataData[ipxl++] = 0;
	    imageDataData[ipxl++] = 16;
	    imageDataData[ipxl++] = 0;
	  }
	  ipxl++;
	  mask >>= 1;
    ...
    if (opcode==0xcd || (opcode&0xc7) == 0xc4) { // one of the nine calls
	var s = hex4(z80.pc)
	if (calls[s] == undefined) calls[s] = 1; else calls[s]++;
    }
	...
	var opcode2;
	tstates += 4;
	opcode2 = readbyte_internal( z80.pc & chrmsk );
	z80.pc++;
	z80.pc &= 0xffff;
	z80.r = (z80.r+1) & 0x7f;
	switch(opcode2) {
	...
	z80.pc &= 0xffff;
	tstates += ( 4 );
	opcode3 = readbyte_internal( z80.pc & chrmsk );
	z80.pc++;
	z80.pc &= 0xffff;
	switch(opcode3) {
...
case 0xf5: copy(); break;
case 0xfc: load_t((z80.l | (z80.h << 8))); break;
case 0xfd: save_t((z80.l | (z80.h << 8))); break;
case 0xfe: lprint((z80.l | (z80.h << 8))); break;
